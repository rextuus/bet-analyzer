...
class WeekdayStatisticCommand extends Command
{
    ...
    public function __construct(
        ...
        private readonly SimulatorDetailStatisticService $simulatorDetailStatisticService,
    )
    {
        parent::__construct();
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $simulatorStatistics = $this->simulatorDetailStatisticService->findByWeekdayOrderedDesc(
            $day,
            $maxSimulators * 10
        );

        foreach ($simulatorStatistics as $simulatorStatistic) {
            $skip = false;
            if ($excludeWeekdaySpecific) {
                foreach (Weekday::cases() as $weekday) {
                    if (str_contains($simulatorStatistic->getSimulator()->getIdentifier(), $weekday->name)) {
                        $skip = true;
                    }
                }
            }
...
//        dd(count($simulatorStatistics));
        ...
        $this->createDuplicatesForTopSimulators($simulatorsByCash, $maxSimulators, $output, $isDryRun, $day);
...
//     old variant
    ...
    private function createDuplicatesForTopSimulators(
        array $simulatorsByCash,
        int $maxSimulators,
