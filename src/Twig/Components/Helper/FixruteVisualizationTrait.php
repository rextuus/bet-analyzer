...
trait FixruteVisualizationTrait
{
    private function calculateHtml(
        MatchInfo $matchInfo,
        string $startedClass,
        DateTime $start,
        string $timeDistance,
        ...
        return sprintf(
            ...
            $startedClass,
            $matchInfo->getClassSimulatorWon(),
            $start->format('H:i'),
            $timeDistance,
    ...
    private function getTimeDistance(TipicoBet $fixture): string
    {
        $current = new DateTime();

        $start = (new DateTime())->setTimestamp($fixture->getStartAtTimeStamp() / 1000);
        return SimulationStatisticService::getTimeDistance($start, $current);
    }

    private function getStartedCssClass(TipicoBet $fixture): string
    {
        $current = new DateTime();
    ...
    private function getMatchInfo(
        TipicoBet $fixture,
        bool $isActiveOnWeekDay,
        BetOn $targetBeton,
        float $overUnderTarget
    ): MatchInfo {
        $start = (new DateTime())->setTimestamp($fixture->getStartAtTimeStamp() / 1000);
        ...
        if ($fixture->isFinished()) {
            ...
            if ($result === $targetBeton) {
                $classSimulatorWon = 'simulator-won';
            }

            // over under
            if ($targetBeton === BetOn::OVER && $totalGoals > $overUnderTarget) {
                $classSimulatorWon = 'simulator-won';
            }
            if ($targetBeton === BetOn::UNDER && $totalGoals < $overUnderTarget) {
                $classSimulatorWon = 'simulator-won';
            }

            // both score
            $bothScored = $homeGoals > 0 && $awayGoals > 0;
            if ($targetBeton === BetOn::BOTH_TEAMS_SCORE && $bothScored) {
                $classSimulatorWon = 'simulator-won';
            }
            if ($targetBeton === BetOn::BOTH_TEAMS_SCORE_NOT && !$bothScored) {
                $classSimulatorWon = 'simulator-won';
            }
        ...
        if ($isActiveOnWeekDay) {
            $tipicoUrl = '<a href=https://sports.tipico.de/de/heute/default/event/' . $fixture->getTipicoId(
                ) . ' " target="_blank">Tipico</a>';

            $betanoUrlValue = $fixture->getCorrespondedBetanoBet(
            ) ? 'https://betano.de' . $fixture->getCorrespondedBetanoBet()->getUrl() : 'https://www.betano.de/';
            $betanoUrl = '<a href="' . $betanoUrlValue . '" target="_blank" >Betano</a>';
        } else {
            // print names without href attributes
            $tipicoUrl = '<span>Tipico</span>';
            $betanoUrl = '<span>Betano</span>';
        }
    ...
    private function calculateOddMatrix(Simulator $simulator): array
    {
